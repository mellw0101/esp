...
{
    private:
        void log_flash(const spi_flash_mmap_memory_t &__type)
        {
            #define MMU_PAGE_SIZE_KB 64
            #define KB_TO_MB_DIVISOR 1024
            
            uint32_t freePages = spi_flash_mmap_get_free_pages(__type);
            uint32_t freeFlashSizeKB = freePages *MMU_PAGE_SIZE_KB;
            float freeFlashSizeMB = (float)freeFlashSizeKB / KB_TO_MB_DIVISOR;

            if (__type == SPI_FLASH_MMAP_INST) ESP_LOGI(__func__, "Free Instruction Memory Size %.2f MB", freeFlashSizeMB);
            if (__type == SPI_FLASH_MMAP_DATA) ESP_LOGI(__func__, "Free Flash Memory Size: %.2f MB", freeFlashSizeMB);
        }
    ...
    public:
        ...
        #define INST 1
        #define FLASH 2
        void get_flash_size(const int &__type)
        {
            if (__type & INST ) log_flash(SPI_FLASH_MMAP_INST);
            if (__type & FLASH) log_flash(SPI_FLASH_MMAP_DATA);    
        }

        void create_task(TaskFunction_t callback)
        {
            xTaskCreate(callback, __func__, 4096, nullptr, 5, nullptr);
        }
...
extern "C" void app_main()
{
    ...
    wifi->esp_connect();
    esp->get_flash_size(FLASH | INST);
    
    // telnet_server->init();

    esp->create_task(telnet_server->run_task);
    // while (true)
    // {
    //     telnet_server->run();
    // }
